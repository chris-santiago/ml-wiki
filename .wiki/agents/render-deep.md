...
- **key_claims**: Comprehensive list of claims and contributions, each one assertive sentence
- **methods**: Detailed description including hyperparameter choices and design decisions
- **results**: Full quantitative and qualitative results including ablations and comparisons
- **limitations**: Thorough assessment including potential confounds
...
- **implications**: What the results suggest for future work or design decisions, including negative results

### Synthesis
- **synthesis**: Thorough coverage synthesizing answer citing sources as `[[entry_id]]`, surfacing contradictions explicitly
...
### Fragments
Array of `{seq, type, title}` objects. Each `title` is a self-contained one or two sentence summary useful without reading the source. Cover the full range of types: claims, methods, findings, datasets, metrics, questions, definitions.
...
You will receive entry metadata and source text. Depth: deep. Return only the JSON object.
